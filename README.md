...
5. **사이트맵/robots 생성** ? `npm run build`를 실행하면 postbuild 훅이 자동으로 `public/sitemap*.xml`과 `robots.txt`를 만듭니다. Search Console에 `https://www.novasingerai.com/sitemap.xml`을 제출하세요.
